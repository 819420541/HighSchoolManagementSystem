...
<body>
		...
		<form action="${pageContext.request.contextPath}/database/schools/list.action" 
    		...
    		<div class="col-sm-3">
    			<select class="form-control input-sm" onchange="query(this)" name="condition">
		        	...
		        	<option value="schoolid">学校编号</option>
			...
			<input type="button" class="btn btn-success" value="添加" 
					onClick="javascript:window.location='${pageContext.request.contextPath}/database/schools/addLoad.action'"/>
		</form>
	</div>
	...
	<div class="row" style="padding:15px; padding-top:0px; ">
	    ...
	    <display:table class="table table-condensed table-striped" name="list" pagesize="10" 
			...
			<display:column property="schoolid" 
					title="学校编号"></display:column>
			<display:column property="schoolname" 
					title="学校名称"></display:column>
			<display:column href="${ pageContext.request.contextPath }/database/schools/updateLoad.action" 
					paramId="schoolid" paramProperty="schoolid" value="修改" title="修改"></display:column>
			<display:column href="${ pageContext.request.contextPath }/database/schools/delete.action" 
					paramId="schoolid" paramProperty="schoolid" value="删除" title="删除"></display:column>
	    </display:table>
	</div>
